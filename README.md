...
A feature-rich wireless security scanner and sniffer for the **ESP32-2432S028R (CYD — Cheap Yellow Display)**. Born from the fusion of three prior projects, rebuilt and expanded into a clean hacker-terminal UI with seven independent scanning modes.
...
All seven modes are accessible from the touch footer bar at the bottom of the screen. Tap any label to switch instantly.
...
### `[HASH]` — WPA2 Handshake / EAPOL Capture
Passive WPA2 handshake capture mode — a CYD port of the [ESP32 WiFi Hash Monster](https://github.com/G4lile0/ESP32-WiFi-Hash-Monster) by **@G4lile0**. Captures EAPOL frames from WPA2 4-way handshakes and saves them as PCAP files to SD card for offline cracking with `hashcat` or `Wireshark`.

- Sniffs **all** 802.11 frames (data + management) in promiscuous mode
- Auto-hops channels 1–13 every 500ms to maximize capture coverage
- Tracks APs from beacon frames (builds a MAC → SSID lookup table, up to 80 APs)
- Detects EAPOL frames (EtherType `0x888E`) and correlates with known SSID
- Writes raw 802.11 packets to `/hashNNNN.pcap` (standard PCAP format, link type 105)
- **Live graphs** (updated every second, 280-second history):
  - **Top**: green packets/sec bar chart with auto-scaled Y axis
  - **Bottom**: scrolling dot chart — RSSI (yellow), EAPOL captures (green), deauth frames (red)
  - **Right strip**: current RSSI · total EAPOL · total deauth · AP count
  - **Footer line**: last seen beacon SSID + RSSI + MAC · last captured EAPOL SSID + MAC
- Header: `CH:xx AP:xx Pk:xxxx E/D:x/x SD:On`
- RGB LED flashes **green** on each EAPOL capture
- Tap body to reset capture counters
- PCAP files are compatible with `hashcat --hash-type 22000` after conversion with `hcxpcapngtool`
...
├──────────────────────────────────────────┤
│                                          │
│           mode content                   │  ← Body (192px)
│                                          │
├──────────────────────────────────────────┤
│SCAN│PROBE│CHAN│DAUTH│BLE│SHADY│HASH      │  ← Footer touch bar (28px)
└──────────────────────────────────────────┘
```

- **Green-on-black** hacker terminal theme throughout
- Active mode tab highlighted with green underline
- RGB LED (active LOW): red=deauth alert · blue=BLE · yellow=shady · green=touch/EAPOL
...
[HASH] SSID:"MyNetwork" BSSID:AA:BB:CC:DD:EE:FF EAPOL#1
```

HASH mode additionally writes raw PCAP files (`/hashNNNN.pcap`) containing captured 802.11 frames. SD card is optional — the scanner runs fully without one.
...
This project is a fusion of three prior builds:
...
| **Mar 2026 v1** (current `src/`, 6-mode) | Full rewrite merging both projects: 6-mode scanner, AntiPMatrix-style SCAN display, fixed async scan engine, SD logging, RGB LED, touch UI |
| **Mar 2026 v2** (current `src/`, 7-mode) | Added `[HASH]` mode — a CYD port of ESP32 WiFi Hash Monster |

The `OriginalPredDetectorJan2026/` folder is preserved in this repo as the pre-merge reference.

### Third-party credits

- **[ESP32 WiFi Hash Monster](https://github.com/G4lile0/ESP32-WiFi-Hash-Monster)** by [@G4lile0](https://github.com/G4lile0)  
  The `[HASH]` mode is a direct port of this project's EAPOL detection logic, channel-hopping strategy, PCAP capture, and graph display layout. The original was built on [PacketMonitor32](https://github.com/spacehuhn/PacketMonitor32) by [@spacehuhn](https://github.com/spacehuhn), with contributions from [@tobozo](https://github.com/tobozo) and [@scriptguru](https://github.com/scriptguru).

---

## Legal Notice

This tool is intended for **educational and authorized security research purposes only**. Only use on networks and devices you own or have explicit permission to test. Passive scanning (SCAN, PROBE, CHAN, BLE, HASH) is generally legal; active interference is not. The authors assume no responsibility for misuse.
